...
=======
    ...
    const [page, setPage] = useState()
    const [data, setData] = useState([])
    const [toReviewQty, setToReviewQty] = useState(0)

    const fetchKnowledges = async () => {
        const response = await getAllKnowledges(filterTitle, elementsValue, page)
    ...
    useEffect(() => {
        fetchKnowledges()
    }, [filterTitle, page, elementsValue])

    useEffect(() => {
        setToReviewQty(toReview())
    }, [])
